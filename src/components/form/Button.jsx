...
export default function Button({ children, onClick, type = "button", variant = "primary", disabled = false, className = "" }) { // Lägg till className prop
  ...
  const variants = {
    ...
    destructive_outline: "border border-destructive text-destructive hover:bg-destructive hover:text-white disabled:opacity-70"
  };
  ...
  const focusRingColors = {
    ...
    destructive_outline: "focus:ring-destructive"
  }
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`${baseStyles} ${variants[variant]} ${focusRingColors[variant]} ${disabled ? 'cursor-not-allowed' : ''} ${className}`} // Lägg till externa className här
    >
      {children}
    </button>
  );
